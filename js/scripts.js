document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            const subject = document.getElementById('subject')?.value || 'Hair Products Inquiry';
            const message = document.getElementById('message').value;
            const telegramRecipient = document.getElementById('telegramRecipient').value;
            ...
            // Send to Telegram using Telegram API via a proxy service
            // Note: In a real implementation, you'd use a server-side proxy to hide your bot token
            // This is a simplified demonstration
            const telegramUrl = `https://api.telegram.org/bot123456:YOUR_BOT_TOKEN/sendMessage?chat_id=${telegramRecipient}&text=${encodeURIComponent(formattedMessage)}`;
            
            // For demonstration, show success message (in a real scenario, you'd check the API response)
            showNotification('success', 'Message Sent!', 'Your message has been received. We will contact you soon!');
            contactForm.reset();
            
            console.log('Message would be sent to Telegram:', formattedMessage);
            console.log('To implement the actual Telegram integration, you need to:');
            console.log('1. Create a Telegram Bot using BotFather');
            console.log('2. Replace YOUR_BOT_TOKEN with your actual bot token');
            console.log('3. Use a server-side proxy to hide your bot token from client-side code');
            console.log('4. Set up the proper chat_id (usually the Telegram user ID of the recipient)');            
        });
    }
